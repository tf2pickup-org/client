...
export const queueRequiredPlayerCount = createSelector(
  ...
  queueConfig,
  (classes, config) =>
    classes?.reduce((prev, curr) => prev + curr.count, 0) * config.teamCount,
);

export const queueSlots = createSelector(
  queueFeature,
...
export const queueSlotsForClass = (gameClass: string) =>
  createSelector(queueSlots, slots =>
    slots?.filter(s => s.gameClass === gameClass),
  );

export const slotById = (slotId: number) =>
  createSelector(queueSlots, slots => slots.find(s => s.id === slotId));
...
export const mapVoteResults = createSelector(
  queueFeature,
  feature => feature.mapVoteResults,
);
export const mapVoteTotalCount = createSelector(mapVoteResults, results =>
  results.reduce((a, b) => a + b.voteCount, 0),
);

export const mapVote = createSelector(queueFeature, feature => feature.mapVote);
export const isPreReadied = createSelector(
  queueFeature,
  feature => feature.preReady,
);

export const substituteRequests = createSelector(
  queueFeature,
  feature => feature.substituteRequests,
);

export const queueFriendships = createSelector(
  queueFeature,
  feature => feature.friendships,
);
