import { Component, OnInit, ChangeDetectionStrategy, OnDestroy, ChangeDetectorRef, ElementRef, ViewChild, AfterViewInit } from '@angular/core';
...
export class DocumentEditComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    this.documentEditField = new MDCTextField(this.documentEditControl.nativeElement);
  }
  ...
  save() {
    ...
    this.documentsService.saveDocument(this.name.value, this.body.value).pipe(
      tap(document => this.form.reset({ body: document.body, name: document.name })),
    ).subscribe(() => {
